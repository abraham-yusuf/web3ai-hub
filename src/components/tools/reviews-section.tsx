  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/tools/${toolSlug}/reviews`, {
        ...
        body: JSON.stringify({ rating, title, content, userId }),
      })
...
export function ReviewsSection({ toolSlug, isLoggedIn = false, userId }: ReviewsSectionProps) {
  ...
  async function voteHelpful(reviewId: string) {
    ...
    try {
      const res = await fetch(`/api/tools/${toolSlug}/reviews/${reviewId}/helpful`, {
        method: "POST",
      })
      if (res.ok) {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="h-8 w-8 rounded-full bg-muted flex items-center justify-center text-sm font-medium">
                  {review.user?.name?.[0] || review.user?.username?.[0] || review.userName?.[0] || "?"}
                </div>
                <div>
                  <p className="font-medium">
                    {review.user?.name || review.user?.username || review.userName || "Anonymous"}
                    {review.verified && (
                      <span className="ml-1 text-xs text-green-600" title="Verified User">✓</span>
                    )}
                  </p>
